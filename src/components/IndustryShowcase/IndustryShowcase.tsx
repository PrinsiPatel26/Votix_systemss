...
export function IndustryShowcase() {
  return (
    <section id="industries" className="w-full bg-white py-[50px] sm:py-[60px] md:py-[88px] lg:py-[140px]">
      <div className="mx-auto max-w-content px-4 sm:px-6 md:px-10">
        <div className="flex items-center gap-4 border-t border-line pt-4">
          <span className="text-[10px] sm:text-[11px] font-medium uppercase tracking-label text-steel">05 / Industries</span>
        </div>

        <div className="mt-8 sm:mt-12 flex flex-col justify-between gap-6 sm:gap-8 lg:mt-16 lg:flex-row lg:items-end">
          <WordReveal
            as="h2"
            text={'Engineered for\nyour industry'}
          ...
          <Reveal className="max-w-sm">
            ...
            <TextLink to="/industries" className="mt-4 sm:mt-6">
              All industries
            </TextLink>
          </Reveal>
        </div>

        <div className="mt-8 sm:mt-12 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:mt-16 lg:grid-cols-12 lg:gap-5">
          {industries.map((ind, i) =>
          <Reveal key={ind.slug} delay={Math.min(i * 0.04, 0.2)} className={cn('h-[280px] sm:h-[300px] md:h-[380px]', layout[ind.slug])}>
              <Link
              ...
              aria-label={`${ind.name} industry`}>
                ...
                <span className="absolute left-4 top-4 sm:left-6 sm:top-6 text-[9px] sm:text-[10px] font-medium tabular-nums tracking-label text-white/70">
                  {ind.index}
                </span>
                <span className="absolute inset-x-4 bottom-4 sm:inset-x-6 sm:bottom-6">
                  <span className="block font-display text-[18px] sm:text-[24px] font-semibold tracking-[-0.02em] text-white md:text-[30px]">
                    {ind.name}
                  </span>
                  <span className="mt-1 sm:mt-2 block max-w-sm translate-y-2 text-[12px] sm:text-[13.5px] leading-snug text-white/0 transition-all duration-300 ease-expo group-hover:translate-y-0 group-hover:text-white/80">
                    {ind.summary}
                  </span>
                  <span className="mt-3 sm:mt-4 inline-flex items-center gap-2 text-[10px] sm:text-[11px] font-medium uppercase tracking-label text-white opacity-0 transition-opacity duration-300 group-hover:opacity-100">
                    View industry
                    <ArrowRightIcon className="h-3.5 w-3.5 sm:h-4 sm:w-4 transition-transform duration-200 ease-expo group-hover:translate-x-1" aria-hidden />
                  </span>
                </span>
              </Link>
            </Reveal>
